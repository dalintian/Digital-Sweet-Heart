 ...
 * single-user tool.
 *
 * @module @deepseek-ai/dsh-client-ui-girlfriend
 */
import { mkdir, readFile, readdir, rm, stat, writeFile } from 'node:fs/promises';
import { homedir } from 'node:os';
import { basename, dirname, extname, join, resolve } from 'node:path';
/** Cordis plugin name. */
export const name = 'client-ui-girlfriend-host';
/** Required services: the route registry. */
export const inject = ['webServer'];
/** The base path every route of this plugin lives under. */
export const ROUTE_PREFIX = '/girlfriend';
/**
...
function dataDir() {
    ...
    return join(home, 'storages', 'girlfriend');
}
...
async function handleChat(body) {
    ...
    const response = await fetch(apiUrl(baseUrl, 'chat/completions'), {
        ...
        body: JSON.stringify({
            model,
            messages,
            temperature: num(body.temperature, 0.8),
            max_tokens: body.maxTokens === undefined ? undefined : num(body.maxTokens, 2048),
            stream: false,
        }),
    });
    const text = await response.text();
    if (!response.ok) {
        throw new Error(`模型接口返回 HTTP ${response.status}: ${text.slice(0, 500)}`);
    }
    let data;
    try {
    ...
    if (typeof content !== 'string') {
        throw new Error('模型接口响应缺少 choices[0].message.content');
    }
...
async function handleImage(body) {
    ...
    const text = await response.text();
    if (!response.ok) {
        throw new Error(`生图接口返回 HTTP ${response.status}: ${text.slice(0, 500)}`);
    }
    let data;
    try {
        data = JSON.parse(text);
    ...
    if (first === undefined)
        throw new Error('生图接口响应缺少 data[0]');
    if (typeof first.b64_json === 'string') {
        return { ok: true, dataUrl: `data:image/png;base64,${first.b64_json}` };
    }
    if (typeof first.url === 'string') {
        return { ok: true, dataUrl: await remoteToDataUrl(first.url) };
    }
...
function findVideoUrl(value) {
    ...
    const record = value;
    for (const key of ['url', 'video_url', 'media_url', 'result']) {
...
function findTaskId(value) {
    if (value === null || typeof value !== 'object' || Array.isArray(value))
        return undefined;
    const record = value;
    for (const key of ['id', 'taskId', 'task_id', 'requestId', 'request_id']) {
...
async function handleVideo(body) {
    ...
    const pollIntervalMs = num(body.pollIntervalMs, 3000);
    const maxWaitMs = num(body.maxWaitMs, 300000);
    const submit = async () => {
        ...
        const text = await response.text();
        if (!response.ok) {
            throw new Error(`文生视频接口返回 HTTP ${response.status}: ${text.slice(0, 500)}`);
        }
        try {
    ...
    if (taskId === undefined) {
        throw new Error('文生视频接口响应中没有可直接播放的 url，也没有任务 id');
    }
    const deadline = Date.now() + maxWaitMs;
    for (;;) {
        await new Promise(resolve => setTimeout(resolve, pollIntervalMs));
        const response = await fetch(apiUrl(baseUrl, `${pollPath}/${encodeURIComponent(taskId)}`), {
            headers: authHeaders(apiKey),
        });
        if (!response.ok) {
            throw new Error(`任务查询失败: HTTP ${response.status} ${(await response.text()).slice(0, 300)}`);
        }
        let polled;
        try {
        ...
        const status = typeof polled?.status === 'string'
            ? String(polled.status).toLowerCase()
            : '';
        if (status === 'success' || status === 'completed' || status === 'succeeded' || status === 'done') {
            const url = findVideoUrl(polled);
            if (url === undefined)
                throw new Error('任务已完成，但响应中没有视频 url');
            return { ok: true, url };
        }
        if (status === 'failed' || status === 'failure' || status === 'error' || status === 'canceled') {
            throw new Error(`视频生成任务失败: 状态 ${status}`);
        }
        if (Date.now() > deadline) {
            throw new Error('等待视频生成超时(默认 5 分钟)，可稍后在视频设置中调整轮询参数');
        }
    }
}
        ...
        handler: async (req, res) => {
            try {
                ...
                if (req.method === 'GET' && pathname.startsWith('/girlfriend/assets/')) {
                    await serveAsset(res, decodeURIComponent(pathname.slice('/girlfriend/assets/'.length)));
                    return;
                }
                if (req.method !== 'POST') {
                ...
                switch (pathname) {
                    ...
                    case '/girlfriend/fs/list': {
                        ...
                        for (const entry of entries) {
                            ...
                            names.push({ name: entry.name, updatedAt: info.mtimeMs });
                        }
                    ...
                    case '/girlfriend/fs/read': {
                        ...
                        result = { ok: true, content: await readFile(path, 'utf8') };
                        break;
                    }
                    case '/girlfriend/fs/write': {
                        ...
                        result = { ok: true, name: basename(path) };
                        break;
                    }
                    case '/girlfriend/fs/asset': {
                        ...
                        result = { ok: true, name: safeName(String(body.name)).split('/').pop() };
                        break;
                    }
                    ...
                    default:
                        json(res, 404, { ok: false, message: `unknown route ${pathname}` });
                        return;
                }
                json(res, 200, result);
            }
